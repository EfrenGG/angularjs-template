function relationshipFieldController($log, $filter, $translate, toastrService, httpCommonsService, CVE_APLICACION) {
    var ctrl = this;
    ...
    ctrl.submit = () => {
        ...
        if (methodName) {
            httpCommonsService[methodName](ctrl.subformMetadata.urlApiForma, ctrl.unsavedModel)
                .then(response => onSuccessResponse(response)).catch(error => onErrorResponse(ctrl.action, error));
        }
    };
    ...
    const setSubformMetadata = form => {
        ...
        httpCommonsService.obtenRegistro('infForma', {
            ...
            if (ctrl.subformMetadata.cveForma) {
                if (ctrl.subformMetadata.bCrea || ctrl.subformMetadata.bEdita || ctrl.subformMetadata.bBorra) {
                    ctrl.subformMetadata.bCrud = true;
                }
                // set export url
                ctrl.subformMetadata.exportUrl = ctrl.subformMetadata.urlApiForma + '/export';
                setFields(ctrl.subformMetadata.detallesForma);
                getData();
            }
            // get i18n
            $translate(ctrl.subformMetadata.cveForma + '.' + ctrl.subformMetadata.cveTituloForma)
    ...
    const setFields = details => {
        ...
        ctrl.fields.forEach(field => {
            ...
            if (field.bCrea) {
                let newField = angular.copy(field);
                if (newField.nomCampoDependiente) {
            ...
            if (field.bEdita) {
                let newField = angular.copy(field);
                if (newField.bPk || newField.nomCampoDependiente) {
                    newField.isDisabled = true;
                }
                ctrl.editFields.push(newField);
            }
            if (field.nomCampoDependiente) {
    ...
    const getData = () => {
        httpCommonsService.obtenRegistros(ctrl.subformMetadata.urlApiForma, getDependantValues(), ctrl.gridPaginationOpts.pageNumber, ctrl.gridPaginationOpts.pageSize)
            .then(response => {
    ...
    const getDependantValues = () => {
        ...
        ctrl.dependentFields.forEach(dependent => {
            dependantValues[dependent.fieldName] = ctrl.parentModel[dependent.fieldDependent];
        });
